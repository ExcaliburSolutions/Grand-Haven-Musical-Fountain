...
public class TimelineController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		// timelineScrollPane.hvalueProperty().addListener(new
		...
		// Duration((timelineScrollPane.getHvalue()/100)*(MusicPaneController.getInstance().getMediaPlayer().getTotalDuration().toSeconds()));
	...
	public void setLabelGridPane() {
		...
		for (int i = 0; i < 14; i++) {
			...
			if (labelNames[i].equals("Custom Channel")) {
				labelArray[i].setOnMousePressed(new EventHandler<MouseEvent>() {
					@Override
